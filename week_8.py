...
def generate_gaussian_mix(num_samples, means, covs,
    ...
    y = np.array([rng.choice(range(len(class_probs)), p=class_probs)
                 for x in range(num_samples)])

    for i in range(num_samples):
        X[i] = rng.multivariate_normal(means[y[i]], covs[y[i]])
...
def gaussian_mix_loglik(X, means, covs, class_probs):
    ...
    for i in X:
        ...
        for j in range(len(class_probs)):
            lik += mvn.pdf(i, means[j], covs[j])*class_probs[j]
        loglik += np.log(lik)
...
def process_args():
    ap = argparse.ArgumentParser(
        description='week 4 coursework script for COMP0088')
    ap.add_argument(
        '-s', '--seed', help='seed random number generator', type=int, default=None)
    ap.add_argument('-n', '--num_samples',
                    help='number of samples to use', type=int, default=100)
    ap.add_argument('-g', '--num_gaussians',
                    help='number of gaussians to generate', type=int, default=2)
    ap.add_argument('-f', '--fit_gaussians',
                    help='number of gaussians to fit', type=int, default=None)
    ap.add_argument('-m', '--max_iter',
                    help='max number of iterations to fit', type=int, default=10)
    ap.add_argument('-k', '--num_hidden',
                    help='number of hidden HMM states', type=int, default=3)
    ap.add_argument(
        '-d', '--dwell', help='bias towards remaining in same state', type=float, default=2)
    ap.add_argument(
        '-N', '--noise', help='noise scaling on HMM observations', type=float, default=0.3)

    ap.add_argument('file', help='name of output file to produce',
                    nargs='?', default='week_8.pdf')
...
if __name__ == '__main__':
    ...
    means = [rng.random(2) * (LIMITS[1] - LIMITS[0]) + LIMITS[0]
    ...
    class_probs = rng.random(args.num_gaussians) + \
        np.ones(args.num_gaussians) * 0.5
    class_probs /= np.sum(class_probs)

    X, y = generate_gaussian_mix(
        args.num_samples, means, covs, class_probs, rng)
    ...
    if X is None:
        ...
        for ii in range(args.num_gaussians):
            ...
            axs[0, 0].contour(xx, yy, pdf, origin='lower',
                              extent=extent, alpha=0.5, colors=[DARK[ii]])
            axs[0, 0].scatter(means[ii][0], means[ii][1],
                              s=100, color=DARK[ii], marker='x')
        ...
        print(
            f'Q2: attempting to fit data with {args.fit_gaussians} Gaussians')
        ...
        if resps is None:
            ...
            yy = np.linspace(bottom, top, 100)
            grid = np.moveaxis(
                np.stack(np.meshgrid(xx, yy, indexing='xy')), 0, -1)
            extent = (left, right, bottom, top)

            for ii in range(args.fit_gaussians):
                pdf = mvn.pdf(grid, mean=fit_means[ii], cov=fit_covs[ii])
                axs[0, 2].contour(xx, yy, pdf, origin='lower',
                                  extent=extent, alpha=0.5, colors=[DARK[ii]])
                axs[0, 2].scatter(fit_means[ii][0], fit_means[ii]
                                  [1], s=100, color=DARK[ii], marker='x')
    ...
    transitions = rng.random(
        (args.num_hidden, args.num_hidden)) + np.eye(args.num_hidden) * args.dwell
    ...
    if x is None:
        ...
        utils.plot_unimplemented(
            axs[1, 1], title=f'Viterbi Decoding (True Model)')
        utils.plot_unimplemented(
            axs[1, 2], title=f'Viterbi Decoding (Fitted Model)')
    else:
        ...
        fit_z = viterbi(x, initial_probs, transitions,
                        emission_means, emission_sds)
        ...
        if hmm is None:
            ...
            utils.plot_unimplemented(
                axs[1, 2], 'Viterbi Decoding (hmmlearn fit)', msg='hmm learn unavailable')
        else:
