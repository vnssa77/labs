...
class CNN(Model):
    ...
    def __init__(self, input=INPUT_SHAPE, spec=DEFAULT_CNN, output=NUM_CLASSES,
        ...
        layers = [nn.Conv2d(input[0], spec[0], kernel, stride, padding), nn.ReLU()]
...
def process_args():
    ap = argparse.ArgumentParser(
        description='week 6 coursework script for COMP0088')
    ap.add_argument(
        '-s', '--seed', help='seed random number generator', type=int, default=None)

    # model configuration
    ap.add_argument('-M', '--model',
                    help='type of model to build', default='linear')
    ap.add_argument('-y', '--layers',
                    help='comma-sep list of hidden layer sizes', default=None)
    ap.add_argument('-k', '--kernel',
                    help='convolution kernel size', default=3, type=int)
    ap.add_argument('-S', '--stride', help='convolution stride',
                    default=2, type=int)
    ap.add_argument('-p', '--padding',
                    help='convolution padding', default=1, type=int)

    # training configuration
    ap.add_argument('-b', '--batch',
                    help='minibatch size to train', default=25, type=int)
    ap.add_argument('-e', '--epochs',
                    help='number of epochs to train', default=5, type=int)
    ap.add_argument('-l', '--lr', help='learning rate',
                    type=float, default=1e-2)
    ap.add_argument('-o', '--optim',
                    help='which optimiser to use', default='sgd')
    ap.add_argument('-m', '--momentum', help='learning momentum',
                    default=0.9, type=float)
    ap.add_argument(
        '-d', '--decay', help='weight decay (L2 regularisation)', default=0, type=float)
    ap.add_argument('-c', '--classes',
                    help='how many classes to use', default=0, type=int)
    ...
    ap.add_argument(
        '-D', '--data', help='name of dataset to load', default='usps')

    ap.add_argument('file', help='name of output file to produce',
                    nargs='?', default='week_6.pdf')
...
if __name__ == '__main__':
    ...
    t1 = perf_counter()
    num_classes = min(
        args.classes, NUM_CLASSES) if args.classes > 1 else NUM_CLASSES

    if args.model.lower() == 'mlp':
        print('building MLP with hidden layers: ' + str(layers))
        model = MLP(input=input_size, spec=layers,
